...
Template.TEMPLATE_NAMEDirs.events({
	"click .tree-view-dir-link": function(e, t) {
		...
		e.preventDefault();
		Meteor.call("COLLECTION_UPDATE_METHOD", this._id, { expanded: !this.expanded });
		...
		bootbox.dialog({
			...
			buttons: {
				success: {
					...
					callback: function() {
						Meteor.call("COLLECTION_REMOVE_METHOD", self._id, function(e, r) {
							TEMPLATE_NAMEGotoFirstLink();
						});
					}
				},
...
Template.TEMPLATE_NAMEFilenameModal.events({
	"submit": function(e, t) {
		...
		function submitAction(result, msg) {
			bootbox.hideAll();
			if(result) {
				Meteor.call("COLLECTION_UPDATE_METHOD", self.data._id, { expanded: true }, function(e) {
					$("a[data-id='" + result + "']").click();
				});
			}
		}
		...
		validateForm(
			...
			function(values) {
				if(self.mode === "rename") {
					...
					Meteor.call("COLLECTION_UPDATE_METHOD", self.data._id, { ITEM_NAME_FIELD: values.filename }, function(e, r) { 
						if(e)
				...
				if(self.mode == "new-file") {
					...
					Meteor.call("COLLECTION_INSERT_METHOD", val, function(e, r) {
						if(e)
							errorAction(e.message);
						else
							submitAction(r); 
