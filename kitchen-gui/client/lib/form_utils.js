...
this.elementCanFocus = function( $el ) {
...
this.validateForm = function(formObject, validationCallback, errorCallback, submitCallback, options) {
	var values = {};
	var error = false;
	formObject.find("input,select,textarea").each(function() {
		...
		var formGroup = inputObject.closest(".field");
		...
		var fieldOptions = (options && fieldName) ? options[fieldName] || {} : {};
		...
		function validationError(errorMessage) {
			...
			if(elementCanFocus(inputObject)) {
				inputObject.focus();
			}
			if(errorLabel) {
		...
		if(!skipValue) {
			// Check required
			if((inputObject.attr("required") || inputObject.attr("data-required") || fieldOptions.required) && !fieldValue) {
			...
			if(inputObject.attr("type") == "checkbox") {
				var checkboxContainer = inputObject.closest(".input-div,.grouped.fields");
				var req = checkboxContainer.attr("data-required") || fieldOptions.required;
				if(req) {
			...
			if(dataType == "URL") {
				if(fieldValue && !isValidURL(fieldValue)) {
					validationError(labelText + ": please enter valid URL starting with http:// or https://");
					return false;
				}
			}
			...
			if(validationCallback) {
				var errorMessage = validationCallback(fieldName, fieldValue, values);
				if(errorMessage) {
